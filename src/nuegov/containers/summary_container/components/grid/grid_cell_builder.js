...
import { DisplayIcon, DisplayIconButton } from "components/display_components";
import GridServices from "../../utils/services";

const CellBuilder = () => {
  const { getRoute } = GridServices();

  const GridCellBuilder = (
    data,
    fieldmeta,
    wholeData = {},
    cellCallBack,
    ...
    const { DateRange, Edit } = SystemIcons;
    ...
    if (
      data !== undefined ||
      (!data && ["LIST", "NUMBER"].includes(fieldmeta.type))
    )
        ...
        case "NUMBER": {
          ...
          return (
            <div style={{ display: "flex" }}>
              {fieldmeta.canUpdate && !disableCellEdit && (
                <DisplayIconButton
                  size="small"
                  ...
                  onClick={(e) =>
                    writeAccess ? cellCallBack(fieldmeta, eachRowData) : null
                  }
                >
                  <Edit
                    style={{
